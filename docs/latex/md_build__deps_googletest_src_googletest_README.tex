\hypertarget{md_build__deps_googletest_src_googletest_README_autotoc_md395}{}\doxysubsubsection{Setup}\label{md_build__deps_googletest_src_googletest_README_autotoc_md395}
To build Google\+Test and your tests that use it, you need to tell your build system where to find its headers and source files. The exact way to do it depends on which build system you use, and is usually straightforward.\hypertarget{md_build__deps_googletest_src_googletest_README_autotoc_md396}{}\doxysubsection{Build with CMake}\label{md_build__deps_googletest_src_googletest_README_autotoc_md396}
...
CMake works by generating native makefiles or build projects that can be used in the compiler environment of your choice. You can either build Google\+Test as a standalone project or it can be incorporated into an existing CMake build for another project.\hypertarget{md_build__deps_googletest_src_googletest_README_autotoc_md397}{}\doxysubsubsection{Standalone CMake Project}\label{md_build__deps_googletest_src_googletest_README_autotoc_md397}
When building Google\+Test as a standalone project, the typical workflow starts with


\begin{DoxyCode}{0}
\DoxyCodeLine{git clone https://github.com/google/googletest.git -\/b v1.16.0}
...
On Mac OS X with Xcode installed, a {\ttfamily .xcodeproj} file will be generated.\hypertarget{md_build__deps_googletest_src_googletest_README_autotoc_md398}{}\doxysubsubsection{Incorporating Into An Existing CMake Project}\label{md_build__deps_googletest_src_googletest_README_autotoc_md398}
...
Note that this approach requires CMake 3.\+14 or later due to its use of the {\ttfamily Fetch\+Content\+\_\+\+Make\+Available()} command.\hypertarget{md_build__deps_googletest_src_googletest_README_autotoc_md399}{}\doxyparagraph{Visual Studio Dynamic vs Static Runtimes}\label{md_build__deps_googletest_src_googletest_README_autotoc_md399}
...
Enabling this option will make gtest link the runtimes dynamically too, and match the project in which it is included.\hypertarget{md_build__deps_googletest_src_googletest_README_autotoc_md400}{}\doxysubsubsection{C++ Standard Version}\label{md_build__deps_googletest_src_googletest_README_autotoc_md400}
An environment that supports C++14 is required in order to successfully build Google\+Test. One way to ensure this is to specify the standard in the top-\/level project, for example by using the {\ttfamily set(\+CMAKE\+\_\+\+CXX\+\_\+\+STANDARD 14)} command along with {\ttfamily set(\+CMAKE\+\_\+\+CXX\+\_\+\+STANDARD\+\_\+\+REQUIRED ON)}. If this is not feasible, for example in a C project using Google\+Test for validation, then it can be specified by adding it to the options for cmake via the{\ttfamily -\/DCMAKE\+\_\+\+CXX\+\_\+\+FLAGS} option.\hypertarget{md_build__deps_googletest_src_googletest_README_autotoc_md401}{}\doxysubsection{Tweaking Google\+Test}\label{md_build__deps_googletest_src_googletest_README_autotoc_md401}
...
We list the most frequently used macros below. For a complete list, see file \href{https://github.com/google/googletest/blob/main/googletest/include/gtest/internal/gtest-port.h}{\texttt{ include/gtest/internal/gtest-\/port.\+h}}.\hypertarget{md_build__deps_googletest_src_googletest_README_autotoc_md402}{}\doxysubsection{Multi-\/threaded Tests}\label{md_build__deps_googletest_src_googletest_README_autotoc_md402}
...
Note\+: while the above steps aren\textquotesingle{}t technically necessary today when using some compilers (e.\+g. GCC), they may become necessary in the future, if we decide to improve the speed of loading the library (see \href{https://gcc.gnu.org/wiki/Visibility}{\texttt{ https\+://gcc.\+gnu.\+org/wiki/\+Visibility}} for details). Therefore you are recommended to always add the above flags when using Google\+Test as a shared library. Otherwise a future release of Google\+Test may break your build script.\hypertarget{md_build__deps_googletest_src_googletest_README_autotoc_md404}{}\doxysubsection{Avoiding Macro Name Clashes}\label{md_build__deps_googletest_src_googletest_README_autotoc_md404}
